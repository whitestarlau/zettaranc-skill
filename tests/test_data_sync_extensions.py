...
def test_data_syncer_has_sync_daily_and_compute_method():
    ...
    from modules.data_sync import DataSyncer
    assert hasattr(DataSyncer, "sync_daily_and_compute"), \
        "DataSyncer.sync_daily_and_compute missing"
    import inspect
...
def test_thin_shells_total_under_250_lines():
    ...
    total = sum(
        (PROJECT_ROOT / s).read_text(encoding="utf-8").count("\n")
...
def test_thin_shell_no_duplicate_compute_xxx(script):
    ...
    forbidden = ["def compute_ma(", "def compute_ema(", "def compute_kdj(",
                 "def compute_rsi(", "def compute_boll(", "def compute_macd("]
    for f in forbidden:
